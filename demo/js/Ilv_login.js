 ...
 * submit(objForm)
***************************************************************************************************/
Ilv_login.submit = function(objForm){
    form = objForm;
    if(form==null){
        form = document.getElementById("ilv_loginForm");
    }
    //regex
    var numRe = /^[0-9]{2}$/; //number between 01 and 99
    var ascRe = /^[A-Za-z_]{1}\w*$/; //standard name of file and so on.
    var notBlankRe = /[\S]{1}/; //at least one noblank character
    var blankRe = /^\s*$/; //a blank line
    var datetimeRe = /^[0-9]{4}-[0-9]{2}-[0-9]{2} [0-9]{2}:[0-9]{2}:[0-9]{2}$/; //datetime regex
    var msg = "";
    var succeed = true;
    var start = document.getElementById("inputLoginStart");
    if(start && start.value.search(datetimeRe)==-1){
        msg += "start time is error in style"+Ilv_login.br;
        start.value = "0000-00-00 00:00:00";
        succeed = false;
    }
    var end = document.getElementById("inputLoginEnd");
    if(end && end.value.search(datetimeRe)==-1){
        msg += "end time is error in style"+Ilv_login.br;
        end.value = "0000-00-00 00:00:00";
        succeed = false;
    }
    var account = document.getElementById("inputLoginAccount");
    if(account && account.value.search(notBlankRe)==-1){
        msg += ""+Ilv_login.br;
        account.value = "";
        succeed = false;
    }
